...
public:
   // constructors
 Point()            : x(0.0), y(0.0), check(false), dead(false), wrap(false) {}
 Point(bool check)  : x(0.0), y(0.0), check(check), dead(false), wrap(false) {}
  ...
  float getYMax()    const { return yMax;           }
   ...
   // setters
  void setWrap(const bool wrap) {this->wrap = wrap; }
   void setX(float x);
   void setY(float y);
